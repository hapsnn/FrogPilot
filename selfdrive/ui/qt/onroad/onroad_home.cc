...
void OnroadWindow::paintEvent(QPaintEvent *event) {
  ...
  if (showTuning) {
    ...
    logicsDisplayString += QString("Friction: %1 | ").arg(liveValid ? friction : "Calculating...");
    logicsDisplayString += QString("Lateral Acceleration: %1").arg(liveValid ? latAccel : "Calculating...");
  }
  if (!logicsDisplayString.isEmpty()) {
    p.save();
